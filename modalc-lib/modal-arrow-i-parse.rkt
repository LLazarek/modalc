...
(struct istx (is-chaperone-contract? args rst pre params ress post mode) #:transparent)
...
;; pull-out-pieces :
;; stx -> (values raw-mandatory-doms raw-optional-doms id/rest-id pre-cond range post-cond mode-expr) 
(define (pull-out-pieces stx)
  (let*-values ([(mode leftover)
                 (syntax-case stx ()
                   [(_ mode-expr . leftover)
                    (values #'mode-expr #'leftover)]
                   [_ (raise-syntax-error #f "expected a contract enforcement mode" stx)])]
                [(is-chaperone-contract? leftover)
                 (syntax-case leftover ()
                   ...
                   [leftover
                    (let ([lst (syntax->list #'leftover)])
                      (when (null? lst)
                        (raise-syntax-error #f "expected a sequence of mandatory domain elements 1"
                                            stx))
                ...
                [(raw-mandatory-doms leftover)
                 (syntax-case leftover ()
                   ...
                   [(a . leftover)
                    (raise-syntax-error #f 
                                        "expected a sequence of mandatory domain elements 2"
                                        stx #'a)]
                   [_
                    (raise-syntax-error #f "expected a sequence of mandatory domain elements 3" stx)])]
