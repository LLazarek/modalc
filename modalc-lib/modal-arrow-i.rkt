...
(provide (rename-out [->i/m modal->i])
  ...
  ((if chaperone? build-chaperone-contract-property build-contract-property)
   ...
   #:name (λ (ctc)
            ...
            (let* ([name-info (->i-name-info ctc)]
              ...
              `(->i ,(object-name (->i-mode ctc))
                    ,(arg/ress->spec args-info
...
(define-for-syntax (mk-wrapper-func/blame-id-info stx an-istx used-indy-vars method?)
  ...
  (values
   ...
   (with-syntax ([arg-checker (or (syntax-local-infer-name stx) 'arg-checker)])
     #`(let ([should-check-ctc? #,(istx-mode an-istx)])
              ...
              (let ([arg-checker
                ...
                (make-keyword-procedure
                 (λ (kwds kwd-args . args)
                   (with-contract-continuation-mark
                     blame+neg-party
                     ;; ll: notice that modes are privileged, the args aren't guarded for them,
                     ;; because doing it the right way is far too much work
                     (if (should-check-ctc? (list kwds kwd-args args))
